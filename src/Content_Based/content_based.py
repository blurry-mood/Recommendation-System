...
credits = pd.read_csv('dataset/lemovielens/credits.csv')
keywords = pd.read_csv('dataset/lemovielens/keywords.csv')
metadata = pd.read_csv('dataset/lemovielens/movies_metadata.csv', low_memory=False)
...
def gather(x):
    return ' '.join(x['keywords']) + ' ' + ' '.join(x['cast']) + ' ' + x['director'] + ' ' + x[
        'producer'] + ' ' + ' '.join(x['genres'])
...
def content_based_model(favrt_movies, top_n=10, metadata=metadata):
    ...
    sim_scores = sim_scores[len(favrt_movies):top_n + 1 + 2]
